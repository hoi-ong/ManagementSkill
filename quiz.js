(function () {
  ...
  function renderQuestions() {
    ...
    SET.forEach(function (q, i) {
      ...
      el.innerHTML =
        '<div class="qhead"><span class="badge">Câu ' + (i + 1) + "/" + total + "</span>" +
    ...
    box.addEventListener("change", onAnswerChange, { once: false });
  }
  ...
  function renderReview() {
    $("review").innerHTML = SET.map(function (q, i) {
      ...
      const ok = answers[i] === q.a;
      return (
        '<div class="q"><div class="qhead"><span class="badge">Câu ' + (i + 1) + "</span>" +
  ...
  async function saveViaApi() {
    ...
    try {
      ...
      const suffix = attempt ? " Đây là lần làm thứ " + attempt + " của bạn." : "";
      status("ok", "Đã lưu kết quả thành công." + suffix);
    } catch (error) {
      console.error("Không lưu được kết quả:", error);
      status("err", "Không lưu được kết quả (" + esc(error.message || String(error)) + "). Vui lòng tải file kết quả bên dưới.");
    }
  }
  ...
  async function postOnce(payload) {
    ...
    const timeoutMs = Number(CONFIG.apiTimeoutMs || 15000);
  ...
  const HEADER = [
    "Thời gian nộp", "Họ tên", "Lớp", "Email", "Lần làm",
    "Điểm", "Tổng câu", "Tỷ lệ %", "Kết quả", "Số phút làm bài"
  ];

  function resultRow() {
    return [
      fmtTime(new Date(result.submittedAt)),
      student.name,
      student.clazz,
      student.email || "",
      1,
